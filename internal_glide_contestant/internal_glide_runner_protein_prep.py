...
class schrodinger_protprep(ProteinPrep):
    ...
    def receptor_scientific_prep(self, 
        ...
        ph = targ_info_dict['ph'][0]
        if not ph.replace('.','').isdigit():
            logging.info('Receptor candidate ' + protein_file + 
                         ' has invalid pH (' + ph + '). Assuming pH = 7.')
            ph = '7'
        
        sch_prep_cmd = ('$SCHRODINGER/utilities/prepwizard -WAIT -propka_pH ' +
                        ph + ' split_receptor1.pdb ' + prepared_protein_file +
                        ' &> prep_output')
